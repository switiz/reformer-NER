...
def load_and_cache_examples(config, tokenizer, mode):
    processor = processors[config.task](config)

    # Load data features from cache or dataset file
    cached_file_name = 'cached_{}_{}_{}_{}'.format(
        config.task, list(filter(None, config.model_name.split("/"))).pop(), config.max_seq_len, mode)

    pad_token_label_id = torch.nn.CrossEntropyLoss().ignore_index
    cached_features_file = os.path.join(config.data_dir, cached_file_name)
    if os.path.exists(cached_features_file):
        ...
        features = convert_examples_to_features(examples, config.max_seq_len, tokenizer, pad_token_label_id=pad_token_label_id)
